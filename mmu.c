...
//Collaborated with Ibukun Esan

enum MMS {FIFO = 1, BESTFIT, WORSTFIT}; 
...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
		...
		int blk_index = list_get_index_of_by_Size(freelist, blocksize);
		block_t* blk = list_remove_at_index(freelist, blk_index);
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
		...
		int blk_index = list_get_index_of_by_Pid(alloclist, pid);
		block_t* blk = list_remove_at_index(alloclist, blk_index);
		blk->pid = 0;
		// add to free list based on policy
		if (policy == FIFO) {
